...
* The fields in the schema are mostly from ``zope.schema``. A reference of available fields is at https://docs.plone.org/4/en/external/plone.app.dexterity/docs/reference/fields.html
* In ``directives.widget(level=RadioFieldWidget)`` we change the default widget for a Choice-field from a dropdown to radioboxes. A incomplete reference of available widgets is at https://docs.plone.org/4/en/external/plone.app.dexterity/docs/reference/widgets.html
...
* ``_sponsors`` is cached for an hour using `plone.memoize <https://docs.plone.org/4/en/manage/deploying/testing_tuning/performance/decorators.html#timeout-caches>`_. This way we don't need to keep all sponsor-objects in memory all the time. We could also cache until one of the sponsors is modified:
...
.. seealso::
    ...
    https://docs.plone.org/4/en/develop/plone/images/content.html#for-plone-4
