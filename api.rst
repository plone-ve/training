...
The most important tool nowadays for plone-developers is the addon `plone.api <https://docs.plone.org/4/en/external/plone.api/docs/index.html>`_ that covers 20% of the tasks any Plone developer does 80% of the time. If you are not sure how to handle a certain task be sure to first check if plone.api has a solution for you.

The api is divided in five sections. Here is one example from each:

* `Content`: `Create content <https://docs.plone.org/4/en/external/plone.api/docs/content.html#create-content>`_
* `Portal`: `Send E-Mail <https://docs.plone.org/4/en/external/plone.api/docs/portal.html#send-e-mail>`_
* `Groups`: `Grant roles to group <https://docs.plone.org/4/en/external/plone.api/docs/group.html#grant-roles-to-group>`_
* `Users`: `Get user roles <https://docs.plone.org/4/en/external/plone.api/docs/user.html#get-user-roles>`_
* `Environment`: `Switch roles inside a block <https://docs.plone.org/4/en/external/plone.api/docs/env.html#switch-roles-inside-a-block>`_
